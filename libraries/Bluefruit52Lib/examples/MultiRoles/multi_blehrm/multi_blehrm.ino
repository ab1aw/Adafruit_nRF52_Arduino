 ...
 * to another peripheral using HRM service.
 * 
 * Mobile <--> DualRole <--> peripheral Ble HRM
 */
#include <bluefruit.h>
...
typedef struct
{
  ...
  BLEClientService        *hrmsClient;
  BLEClientCharacteristic *hrmcClient;
  BLEClientCharacteristic *bslcClient;
} prph_info_t;
...
void setup()
{
  ...
  Serial.println("Bluefruit52 Multi Role HRM Example");
  Serial.println("-------------------------------------\n");
  ...
  Bluefruit.setName("MultiHrm");    // Check bluefruit.h for supported values
